...
- 現在選択中 Document が一覧ウィンドウ外へ退避した場合、左ペインの並び順を崩して固定表示しない。代わりに左ペイン上部へ「選択中 Document へ戻る」導線を表示し、操作時は選択中 Document を含む一覧ウィンドウを再取得する。
...
- 適用ルール
  ...
  - 現在選択中 Document が左ペインの一覧ウィンドウ外にある状態で `J/K` を実行した場合は、現在のスクロール位置ではなく選択中 Document の前後関係を基準に移動し、移動先 Document を含む一覧ウィンドウへ戻す。

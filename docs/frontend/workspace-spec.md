...
- 検索時は空白・アンダースコア・ハイフンの差をならして扱い、ASCII の大文字小文字差は無視する。
- 検索結果一覧では `document_name` に加え、一致箇所を含む本文抜粋を表示し、抜粋内の一致箇所をハイライト表示する。
- 検索中に現在選択中 Document が検索結果から外れた場合でも、中央ペインの表示は維持する。その代わり左ペインで「現在表示中の Doc は検索結果外」であることを案内し、検索クリアまたは現在 Doc 名での再絞り込み導線を出す。
- 検索結果が 0 件の場合は空状態を表示し、少なくとも「検索クリア」と「Doc 作成」へ進める導線を出す。
...
- `関連例` タブは以下で構成する。
  ...
  - `同じラベルの例` セクション: 現在注目中 Label に付いた他表層例を、現在 project 配下の全 Document を横断して表示する。
  - `同一表層の他アノテーション` セクション: 選択中 Annotation の `span_text` に一致する他 Annotation を、現在 project 配下の全 Document を横断して表示する。
- `同じラベルの例` と `同一表層の他アノテーション` は、それぞれ独立した内部スクロール領域を持つ。
